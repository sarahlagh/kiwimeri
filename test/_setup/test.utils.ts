...
import { space, spaceContent, store } from '@/core/db/store';
...
export const getNewParsedContent = (text: string) => {
  return JSON.parse(getNewContent(text));
};
...
export const getRowCountInsideNotebook = (notebook?: string) => {
  ...
  return collectionService.getAllChildren(notebook).length;
};
...
export const getRowIdsInsideNotebook = (notebook?: string) => {
  ...
  return collectionService.getAllChildren(notebook).map(i => i.id);
};
