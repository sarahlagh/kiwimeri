...
class ImportService {
  ...
  private validateMetadataDisplay(
    ...
    if (
      ...
      throw new Error(
        'parsing error: display_opts.sort.by must be a sort field'
      );
    }
  ...
  private validateMetadataFile(value: unknown): asserts value is ZipMetadata {
    if (!isObject(value)) {
    ...
    if ('display_opts' in value) {
      this.validateMetadataDisplay(value.display_opts);
    }
  ...
  private parseMetadataFile(
    ...
    try {
      ...
      this.validateMetadataFile(origMeta);
